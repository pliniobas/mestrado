...
d3 = ( dict (
           date = [(aux[0].strftime('%Y/%m/%d %H')) for aux in p],
           hs = [aux[4] for aux in p],
           tp = [aux[5] for aux in p],
           dp = [aux[7] for aux in p],
           lat = [lat for _ in range(len(p))],
           lon = [lon for _ in range(len(p))],
           latint = [latint for _ in range(len(p))],
           lonint = [lonint for _ in range(len(p))],
           )
        )

f = open('41049w_.Dict.txt','w')
f.write(str(d3))
f.close()
...
df2 = df.loc[:,['hs','tp','dp','lat','lon']]
df2.loc[df2.loc[:,'dp'] == 0,'dp']
any((df2.loc[:,'hs'] == 0).values)
df3 = df[df2.tp > 10]
