...
if __name__ == '__main__':
    ...
    CtrlDirNum = 16 #numero de direções que serão divididos os scatter. Os intervalos calculados ficam armazenados em dirInt (direcao intervalo)
...
#            for CtrlDirQuad in xrange(1,CtrlDirNum,1):
            for dirQuad in CtrlDirQuad:
                ...
                if len(H) > CtrlMinDataLen:
                    ...
                    fig.savefig(os.path.join(dirname,filename))
                    print(time.clock() - t)
                    f = open('Nataf quadrante %d com H ajustado em %s e T ajustado em %s - T= %d'%(dirQuad,aux,aux2,time.clock() - t),'w')
                    # =============================================================================
