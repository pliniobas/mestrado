...
#%%
    ...
    t = time.clock()
    mll = mdc()
    mll.fit(H,T,tipofH='lognormal',tipofT='lognormal',bins=40)
#    mll.rho()
    fig = mll.printContour()
    
    mww = mdc()
    mww.fit(H,T,tipofH='weibull',tipofT='weibull',bins=40)
    fig = mww.printContour()


#    plt.plot(seriebruta.flatten())
...
nat = nataf()
nat.fit(H,T,tipofT = 'lognormal',tipofH = 'weibull',bins=30,rangeH=[0,10],rangeT=[0,20])
#nat.rho()
...
#    sp.check_output(['python',r"C:\Google Drive\0Mestrado Dissertacao\0Trabalhando Geral\teste.py"])
#    
#    
#    p = sp.Popen(['python',r"C:\Google Drive\0Mestrado Dissertacao\0Trabalhando Geral\teste.py"],bufsize=-1,stdin=sp.PIPE,stdout=sp.PIPE,stderr=sp.PIPE)
#    
#    #dHT.to_json()
#    p.communicate('paca\ntatu')
