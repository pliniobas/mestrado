...
#        print(s.fT(h).pdf(t))
    ...
    def printContour(s,**kwargs):
        ...
        ax.set_xlabel('Hs')
        ax.set_ylabel('Tp')
        ax.set_title('%s Hs ajustado em %s e Tp ajustado em %s'%(s.name,s.fHtype,s.fTtype))
        ax.set_xticks(xgrid[0])
        ax.set_xticklabels(xtl)
        ax.grid()
        fig.colorbar(colbar, shrink=0.5, aspect=5)
        ...
        if kwargs.has_key('dirI') and kwargs.has_key('dirM') and kwargs.has_key('dirF'):
            ...
            dirM = kwargs['dirM']
            dirF = kwargs['dirF']
            ax.text(0.05,temp,u'Nº coletas %d || %d° < %s < %d°'%(len(s.H),dirI,dirM,dirF))
            temp -= 0.1
        
        ax.text(0.05,temp,u'rho dados brutos = %.3f'%(scipy.stats.pearsonr(H,T)[0]))
        temp -= 0.06
        
        try:   
            ax.text(0.05,temp,u'rho funcao ajustada = %.4f'%(s.rho))
            temp -= 0.1    
    ...
    def integral(s):
        ran = [[s.H.min()*0.5,s.H.max()*1.5],[s.T.min()*0.5,s.T.max()*1.5]]
    ...
    def fit(s,H,T,**kwargs):
        ...
        if kwargs.has_key('rangeH'):
            s.rangeH = (0,kwargs['rangeH'])
            pass
        if kwargs.has_key('rangeT'):
            s.rangeT = (0,kwargs['rangeT'])
            pass
        ...
        if kwargs.has_key('printfHT'):
            x = np.linspace(0,H.max(),100)
            
            fig = plt.figure()
            fig.add_subplot(211)
            
            dl = pd.DataFrame(ln_param)
            plt.plot(x,s.Tfxi(x),'r', label = 'xi')
            plt.plot(x,s.Tflamb(x),'g',label = 'lamb')
            plt.plot(dl.iloc[:,3].values,dl.iloc[:,0].values,'o',color = 'r')
            plt.plot(dl.iloc[:,3].values,dl.iloc[:,2].values,'o',color = 'g')
            plt.legend()
            
            fig.add_subplot(212)
            dw = pd.DataFrame(wei_param)
            plt.plot(x,s.Tflambw(x),'b',label = 'lambw')
            plt.plot(x,s.Tfalphaw(x),'y',label = 'alphaw')
            plt.plot(dw.iloc[:,3].values,dw.iloc[:,0].values,'o',color ='b')
            plt.plot(dw.iloc[:,3].values,dw.iloc[:,2].values,'o',color ='y')
            plt.legend()
...
#        s.ran = [[3,6],[7,12]]
        ...
        s.rho = (s.mht - s.mh*s.mt) / (s.dph * s.dpt)
        print('calculo rho terminado = ',s.rho )
        print('------------------------------------------')
        return s.rho   
...
#        s.ran = [[1,6],[5,10]]
#
#        funcs = [s.fmh,s.fm2h,s.fmt,s.fm2t,s.fmht]
        
#        s.p.append(Process(target = s.fmh,args = ('',)))        
#        s.p.append(Process(target = s.fm2h,args = ('',)))
#        s.p.append(Process(target = s.fmt,args = ('',)))  
#        s.p.append(Process(target = s.fm2t,args = ('',)))
#        s.p.append(Process(target = s.fmht,args = ('',)))
...
t = time.clock()
mll = mdc()
mll.fit(H,T,tipofH='lognormal',tipofT='lognormal',bins=20,printfHT='')
mll.rho()
mll.printContour()
print time.clock() - t

mww = mdc()
mww.fit(H,T,tipofH='weibull',tipofT='weibull',bins=20,printfHT='')
mww.rho()
mww.printContour()
print time.clock() - t

mlw = mdc()
mlw.fit(H,T,tipofH='lognormal',tipofT='weibull',bins=20,printfHT='')
mlw.rho()
mlw.printContour()
print time.clock() - t

mwl = mdc()
mwl.fit(H,T,tipofH='weibull',tipofT='lognormal',bins=20,printfHT='')
mwl.rho()
mwl.printContour()
print time.clock() - t

    #%%
#ran = [[H.min()*0.8,H.max()*1.2],[T.min()*0.8,T.max()*1.2]]    
#p = Process(target=fmh,args=(mll,ran))    
#p = Process(target=mll.rho,args=())
#p.start()
...
##%%
#p = []    
#p.append(Process(target = temp,args=()))
#p.append(Process(target = temp2,args=()))
#
##%%
#
#
...
#
#%%
#fig = mll.printContour(filename = DirFileList[CtrlfileNum] + 'DQ_%d-%d_'%(dirQuad+1,CtrlDirNum),
#                 directory = DirFigures,
#                 dirI = d[auxT]['di'],
#                 dirM = direcaofinal[d[auxT]['dm']],
#                 dirF = d[auxT]['df'])
#
#
#%%
#DirFigName = DirFigures + (DirFileList[CtrlfileNum]) 
#DirFigName += 'DQ_%d-%d_'%(dirQuad+1,CtrlDirNum)
#DirFigName += '_CONTOUR.png'
#print DirFigName
#fig.savefig(DirFigName)


    #%%

    
def printContour(obj):
    
    #determinando serie distribuicao conjunta / nataf
    obj_hist = obj.pdf_series(obj.H,obj.T)
    
    #determinando serie bruta
    seriebruta, xedges, yedges = np.histogram2d(obj.H, obj.T, bins = obj.bins, range=[obj.rangeH,obj.rangeT], normed=True)
    xedges = xedges[:-1] + xedges[0:2].mean()
    yedges = yedges[:-1] + yedges[0:2].mean()
    xgrid,ygrid = np.meshgrid(xedges,yedges)
    ...
    colbar = ax.contour(obj_hist['xpos'],obj_hist['ypos'],obj_hist['df'] ,cmap=cmap2, vmin = 0.001, vmax = 0.3,levels=levels)
    ax.set_xlabel('Hs')
    ax.set_ylabel('Tp')
    ax.set_title('Funcao %s %s %s'%(obj.name,obj.fHtype,obj.fTtype))
    ax.set_xticks(xgrid[0])
    ax.set_xticklabels(xtl)
    ax.grid()
    fig.colorbar(colbar, shrink=0.5, aspect=5)
    ...
    bias = seriebruta - obj_hist['df']
    colbar = ax.contour(obj_hist['xpos'],obj_hist['ypos'], bias,cmap=cmapbias, vmin = -0.15, vmax = 0.15,levels=levelsBias)
    ax.set_xlabel('Hs')
    ax.set_ylabel('Tp')
    ax.set_title('Bias Hist. Dens. Prob. - PDF (Lognormal)')
    ax.set_xticks(xgrid[0])
    ax.set_xticklabels(xtl)
    ax.grid()
    fig.colorbar(colbar, shrink=0.5, aspect=5)    
...
#        scipy.stats.__init__(self)
    ...
    def run(s):
        ...
        ran = [[s.H.min()*0.8,s.H.max()*1.2],[s.H.min()*0.8,s.T.max()*1.2]]
        print('calculando media H')
    ...
    def integral(s):
        ran = [[0.1,s.H.max()*1.5],[0.1,s.T.max()*1.5]]
        s.i = nquad(lambda h,t: s.pdf(h,t),ran)
        print('calculo de integral terminado = ',s.i)
        return s.i
...
#%%  
            if CtrlAdvCalc:                                                                         
                ...
                #%%
                natll = nataf()
                natll.fit(H,T,tipofH='lognormal',tipofT='lognormal')
                rhonatll = rho_calc(natll,0.1,8,0.1,18)
                
                #%%
                natww = nataf()
                natww.fit(H,T,tipofH='weibull',tipofT='weibull')
                rhonatww = rho_calc(natww,0.1,8,0.1,18)  
    
                #%%
                natlw = nataf()
                natlw.fit(H,T,tipofH='lognormal',tipofT='weibull')
                rhonatlw = rho_calc(natlw,0.1,8,0.1,18)    
                
                
                #%% 
                natwl = nataf()
                natwl.fit(H,T,tipofH='weibull',tipofT='lognormal')
                rhonatwl = rho_calc(natwl,0.1,8,0.1,18)  
             
               
               #%%                 
                mll = mdc()    
                mll.fit(H,T,tipofH='lognormal',tipofT='lognormal')
                rhomll = rho_calc(mll,0.1,8,0.1,18)
                #%%
                
                mww = mdc()
                mww.fit(H,T,tipofH='weibull',tipofT='weibull')
                rhomww = rho_calc(mww,0.1,8,0.1,18)
                #%%
                
                mlw = mdc()
                mlw.fit(H,T,tipofH='lognormal',tipofT='weibull')
                rhomlw = rho_calc(mlw,0.1,8,0.1,18)
                
                #%%
                mwl = mdc()
                mwl.fit(H,T,tipofH='weibull',tipofT='lognormal')
                rhomwl = rho_calc(mwl,0.1,8,0.1,18)
...
#                print('irhonatll')
#                irhonatll = int_calc(natll,0.1,8,0.1,18)                
#                print('irhonatww')
#                irhonatww = int_calc(natww,0.1,8,0.1,18)
#                print('irhonatlw')
#                irhonatlw = int_calc(natlw,0.1,8,0.1,18)
#                print('irhonatwl')
#                irhonatwl = int_calc(natwl,0.1,8,0.1,18)
#                print('imll')
#                imll = int_calc(mll,0.1,8,0.1,18)
#                print('imww')
#                imww = int_calc(mww,0.1,8,0.1,18)
#                print('imlw')
#                imlw = int_calc(mlw,0.1,8,0.1,18)
#                print('imwl')
#                imwl = int_calc(mwl,0.1,8,0.1,18)
                

#%%                
                f = open(DirFigName+'_rho.txt','a')
                f.write('''
                        Para dir = %s 
                        Brutos rho3 %0.6f
                        rho nat ll %0.6f        
                        ...
                        imlw %0.6f
                        imwl %0.6f
                        '''%(auxT,rho3,rhonatll,rhonatww,rhonatlw,rhonatwl,rhomll,rhomww,rhomlw,rhomwl,irhonatll,irhonatww,irhonatlw,irhonatwl,imll,imww,imlw,imwl))
                f.close()
